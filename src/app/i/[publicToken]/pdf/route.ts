...
export async function GET(_: Request, { params }: { params: Promise<{ publicToken: string }> }) {
  ...
  try {
    const bytes = await invoicePdfByToken(publicToken);
    return new Response(new Uint8Array(bytes), {
      headers: { "content-type": "application/pdf", "content-disposition": `inline; filename="factura.pdf"`, "cache-control": "private, no-store" },
    });
  } catch (error) {
